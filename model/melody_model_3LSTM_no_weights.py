...
def melody_model(validation_split=0.2, batch_size=32, epochs=1, nr_files=None, callbacks=False, walltime=0, temp=1.0):
    ...
    temp_save_path = os.path.join(c.project_folder, "data/tf_weights/weights_melody_3nw_saved_wall_time.hdf5")
    ...
    if callbacks:
        ...
        filepath = os.path.join(c.project_folder, "data/tf_weights/m3nw/melody-weights-3LSTMnw-improvement-{epoch:02d}-"
                                                  "vl-{val_loss:0.5f}-vpacc-{val_pitch_output_acc:0.5f}-vlacc-{val_length_output_acc:0.5f}.hdf5")
        # batch_filepath = os.path.join(c.project_folder, "data/tf_weights/melody-weights-improvement-3nw-batch.hdf5")
        ...
        tensorboard = call_backs.TensorBoard(log_dir=os.path.join(c.project_folder, "data/tensorboard_logs/m3nw"))
    ...
    model.fit_generator(generator=melody_data_generator(train_data, batch_size),
                        steps_per_epoch=min(3000,len(train_data) // batch_size),
    ...
    if callbacks and walltime:
        if batches_checkpoint.reached_wall_time:
            ...
            recallParameter = 'qsub -v REDO=True,EPOCH=' + str(
                batches_checkpoint.start_epoch) + ' 3nw_melody_model.sge'
