...
def melody_model(validation_split=0.2, batch_size=32, epochs=1, nr_files=None, callbacks=False, walltime=0, temp=1.0):
    ...
    temp_save_path = os.path.join(c.project_folder, "data/tf_weights/weights_melody_3nwl_saved_wall_time.hdf5")
    ...
    if callbacks:
        ...
        filepath = os.path.join(c.project_folder, "data/tf_weights/m3nwl/melody-weights-3LSTMnwl-improvement-{epoch:02d}-"
    ...
    if callbacks and walltime:
        if batches_checkpoint.reached_wall_time:
            ...
            recallParameter = 'qsub -v REDO=True,EPOCH=' + str(
                batches_checkpoint.start_epoch) + ' 3nwl_melody_model.sge'
