...
def melody_model(validation_split=0.2, batch_size=32, epochs=1, nr_files=None, callbacks=False, walltime=0, temp=1.0):
    ...
    if callbacks:
        ...
        filepath = os.path.join(c.project_folder, "data/tf_weights/melody-weights-1LSTM-improvement-{epoch:02d}-vl-{val_loss:.5f}-pacc-{pitch_output_acc:.5f}-lacc-{length_output_acc:.5f}.hdf5")
    ...
    if callbacks and walltime:
        if batches_checkpoint.reached_wall_time:
            ...
            recallParameter = 'qsub -v REDO=True,EPOCH=' + str(
                batches_checkpoint.last_epoch) + ' 1w_melody_model.sge'
